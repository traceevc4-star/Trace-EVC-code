...
class ResnetBlock(BaseModule):
    ...
    def forward(self, x, mask, time_emb):
        ...
        h += self.mlp(time_emb).unsqueeze(-2)
...
class GradLogPEstimator2d(BaseModule):
    def __init__(
        self, dim, dim_mults=(1, 2, 4), groups=8, pe_scale=1000, spk_emb_dim=0
    ):
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(
                torch.nn.ModuleList(
                    [
                        ResnetBlock(
                            dim_in, dim_out, time_emb_dim=dim, spk_emb_dim=spk_emb_dim
                        ),
                        ResnetBlock(
                            dim_out, dim_out, time_emb_dim=dim, spk_emb_dim=spk_emb_dim
                        ),
                        Residual(Rezero(LinearAttention(dim_out))),
                        Downsample(dim_out) if not is_last else torch.nn.Identity(),
                        Downsample2(dim + spk_emb_dim)
                        if not is_last
        ...
        self.mid_block1 = ResnetBlock(
            mid_dim, mid_dim, time_emb_dim=dim, spk_emb_dim=spk_emb_dim
        )
            ...
            self.ups.append(
                torch.nn.ModuleList(
                    [
                        ...
                        ResnetBlock(
                            dim_in, dim_in, time_emb_dim=dim, spk_emb_dim=spk_emb_dim
                        ),
                        Residual(Rezero(LinearAttention(dim_in))),
                        Upsample(dim_in),
                        Upsample2(dim + spk_emb_dim),
                    ]
                )
            )
    ...
    def forward(self, x, mask, mu, t, spk_emb=None):
        ...
        t = self.mlp(t)
        b, _, l = spk_emb.shape
        t = t.unsqueeze(-1).expand(b, -1, l)
        t = torch.cat((t, spk_emb), dim=1)
        ...
        hiddens = []
        masks = [mask]
        for resnet1, resnet2, attn, downsample, downsample2 in self.downs:
            mask_down = masks[-1]
            ...
            t = downsample2(t * mask_down.squeeze(-2))
        ...
        for resnet1, resnet2, attn, upsample, upsample2 in self.ups:
            mask_up = masks.pop()
            ...
            t = upsample2(t * mask_up.squeeze(-2))
...
class Diffusion(nn.Module):
    def __init__(
        ...
        self.estimator = GradLogPEstimator2d(
            dim, dim_mults=dim_mults, pe_scale=pe_scale, spk_emb_dim=spk_emb_dim
        )
    ...
    def register_beta(self, betas):
        ...
        self.register(
            "sqrt_recip_one_minus_alphas_cumprod", torch.rsqrt(1 - alphas_cumprod)
        )
        self.register(
    ...
    def q_posterior(self, x_0, score, t, eta=1.0):
        ...
        mean = torch.sqrt(
            extract(self.alphas_cumprod_prev, t, score.shape)
        ) * x_0 - torch.sqrt(
            1 - extract(self.alphas_cumprod_prev, t, score.shape) - torch.pow(sigma, 2)
        ) * score * extract(
            self.sqrt_one_minus_alphas_cumprod, t, score.shape
        )
    ...
    def classifier_free_guidance(
        ...
        if text_gradient_scale > 0.0 and spk_gradient_scale > 0.0:
            ...
            score = (
                ...
                + spk_gradient_scale * (score - score_spk_uncon)
            )
        elif text_gradient_scale > 0.0:
    ...
    def reverse_diffusion(
        ...
        for i in range(n_timesteps):
            ...
            alpha_cumprods.append(
                torch.exp(
                    -get_noise(time, self.beta_min, self.beta_max, cumulative=True)
                )
            )
        ...
        for i in range(n_timesteps):
            ...
            score = self.classifier_free_guidance(
                xt,
                mask,
                cond,
                t,
            ...
            mean, var = self.p_mean_variance(
                xt, idx, eta=getattr(self, "infer_eta", 1.0), score=score)
            noise = torch.randn(
                xt.shape, dtype=xt.dtype, device=xt.device, requires_grad=False
            )
    ...
    def forward(
        ...
        return self.reverse_diffusion(
            ...
            spk_emb,
            n_timesteps,
            text_gradient_scale=getattr(self, "infer_text_cfg", text_gradient_scale),
    ...
    def get_trace(
        ...
        return self.reverse_diffusion_with_trace(
            z,
            mask,
            cond,
            spk_emb,
    ...
    @torch.no_grad()
    def reverse_diffusion_with_trace(
        self,
        z,
        mask,
        cond,

...
from .layers import LayerNorm, StyleAdaptiveLayerNorm, MixStyle
...
class EncSALayer(nn.Module):
    def __init__(
        ...
        if num_heads > 0:
            if norm == "ln":
                ...
                self.layer_norm1 = StyleAdaptiveLayerNorm(c, c)
            elif norm == "mixstyle":
                self.layer_norm1 = MixStyle(
                    p=0.5,
                    alpha=0.1,
                    eps=1e-6,
                    hidden_size=c,
        ...
        if norm == "ln":
            ...
            self.layer_norm2 = StyleAdaptiveLayerNorm(c, c)
        elif norm == "mixstyle":
            self.layer_norm2 = MixStyle(
                p=0.5,
                alpha=0.1,
                eps=1e-6,
                hidden_size=c,
    ...
    def forward(self, x, style_vector=None, encoder_padding_mask=None, **kwargs):
        ...
        if self.num_heads > 0:
            ...
            elif self.norm == "mixstyle":
                x = self.layer_norm1(x.transpose(0, 1), style_vector.transpose(0, 1))
                x = x.transpose(0, 1)
            (
                x,
                _,
        ...
        if self.norm == "ln":
            ...
            x = self.layer_norm2(x.transpose(0, 1), style_vector.transpose(0, 1))
            x = x.transpose(0, 1)
        x = self.ffn(x)
        x = F.dropout(x, self.dropout, training=self.training)
        x = residual + x
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, hidden_size, dropout, kernel_size=9, num_heads=2, norm="ln"):
        super().__init__()
...
class FFTBlocks(nn.Module):
    ...
    def forward(
        ...
        hiddens = []
        hiddens.append(x.transpose(0, 1))
